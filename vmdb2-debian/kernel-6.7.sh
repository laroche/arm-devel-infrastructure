...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  RDIR=rpi-linux-$RVER
  if test ! -d $RDIR ; then
    git clone -b rpi-6.7.y https://github.com/raspberrypi/linux/ $RDIR
    test -d $RDIR || exit 1
  else
    pushd $RDIR
  ...
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER d6d6c49dbf4512f1421f5e42896e2d70dc121f9a
  cd ..
  rm -fr $RDIR
fi
...
sed -i -e '1 s/6.7.7-/6.7.9-/' $CDIR/debian/changelog
...
if test "$RPIPATCHES" = 1 ; then
  ...
  rm -f debian/abi/6.7.0-*/arm*
fi
rm -fr debian/abi/6.7.0-*
