...
# Build requirements:
if true ; then
sudo apt-get -qq -y install build-essential fakeroot rsync git #python-debian python3-debian
...
if test $CROSS = 1 ; then
  ...
  sudo apt-get -qq -y install g++-8-aarch64-linux-gnu g++-8-arm-linux-gnueabihf
