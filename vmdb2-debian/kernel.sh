...
if test $CROSS = 1 ; then
  ...
  sudo apt-get -qq -y install g++-12-aarch64-linux-gnu g++-12-arm-linux-gnueabihf
fi
fi
...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER 6c6a6c7e211cc02943dcb8c073919d2105054886
  cd ..
  rm -fr $RDIR
fi
...
sed -i -e '1 s/6.1.177-/6.1.178-/' $CDIR/debian/changelog
...
sed -i -e 's,^bugfix/all/revert-epoll-use-refcount-to-reduce-ep_mutex-content.patch,,g' $CDIR/debian/patches/series
#sed -i -e 's,0056-debugobjects-locking-Annotate-debug_object_fill_pool.patch,,g' $CDIR/debian/patches-rt/series
#exit 0
mkdir -p orig
cd $CDIR || exit 1
test -f ../linux_$KVER.orig.tar.xz || XZ_DEFAULTS="-T 0" debian/bin/genorig.py https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git
