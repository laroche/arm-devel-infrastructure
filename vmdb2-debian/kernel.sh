...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER 76ba310227d2490018c271f1ecabb6c0a3212eb0
  cd ..
  rm -fr $RDIR
fi

if ! test -d $CDIR ; then
  git clone --single-branch --depth 1 -b bookworm-security https://salsa.debian.org/kernel-team/linux.git $CDIR
fi
sed -i -e '/install-rtla)/d' $CDIR/debian/rules.real
# Change Debian source to new version:
sed -i -e '1 s/6.1.27-/6.1.34-/' $CDIR/debian/changelog
