...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER 4631960b4700dd53f5cebb4f7055fd00ccd556ce
  cd ..
  rm -fr $RDIR
fi
...
if ! test -d $CDIR ; then
  ...
  #git clone --single-branch --depth 1 -b 6.5-stable-update https://salsa.debian.org/carnil/linux.git $CDIR
fi
sed -i -e '/install-rtla)/d' $CDIR/debian/rules.real
# Change Debian source to new version:
sed -i -e '1 s/6.5.13-/6.5.13-/' $CDIR/debian/changelog
...
#sed -i -e 's,0001-kernel-fork-beware-of-__put_task_struct-calling-cont.patch,,g' $CDIR/debian/patches-rt/series
#exit 0
mkdir -p orig
cd $CDIR || exit 1
