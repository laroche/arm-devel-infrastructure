...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  RDIR=rpi-linux-$RVER
  if test ! -d $RDIR ; then
    git clone -b rpi-6.19.y https://github.com/raspberrypi/linux/ $RDIR
    test -d $RDIR || exit 1
  else
    pushd $RDIR
    git checkout rpi-6.19.y
    popd
  fi
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER 05f7e89ab9731565d8a62e3b5d1ec206485eeb0b
  cd ..
  rm -fr $RDIR
fi
...
if ! test -d $CDIR ; then
  ...
  #git clone --single-branch --depth 1 -b debian/6.19/forky https://salsa.debian.org/kernel-team/linux.git $CDIR
  #git clone --single-branch --depth 1 -b 6.19-stable-updates https://salsa.debian.org/carnil/linux.git $CDIR
fi
#sed -i -e '/install-rtla)/d' $CDIR/debian/rules.real
# Change Debian source to new version:
sed -i -e '1 s/6.19.1-/6.19.2-/' $CDIR/debian/changelog
...
if test "$RPIPATCHES" = 1 ; then
  ...
  pushd debian/patches
    ...
    rm -f bugfix/rpi/0388-i2c-designware-Add-support-for-bus-clear-feature.patch
    rm -f bugfix/rpi/0389-i2c-designware-Make-the-SDA-hold-time-half-LCNT.patch
  ...
  rm -f debian/abi/6.19.0-*/arm*
fi
rm -fr debian/abi/6.19.0-*
