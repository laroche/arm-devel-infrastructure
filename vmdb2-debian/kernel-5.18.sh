...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  RDIR=rpi-linux-$RVER
  if test ! -d $RDIR ; then
    git clone -b rpi-5.18.y https://github.com/raspberrypi/linux/ $RDIR
    test -d $RDIR || exit 1
  else
    pushd $RDIR
    git checkout rpi-5.18.y
    popd
  fi
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER bc560cecaa8b2517932808fa939e36371ffa036e
  cd ..
  rm -fr $RDIR
fi
...
sed -i -e '1 s/5.18.8-/5.18.11-/' $CDIR/debian/changelog
...
if test "$RPIPATCHES" = 1 ; then
  ...
  rm -f debian/abi/5.18.0-*/arm*
fi
rm -fr debian/abi/5.18.0-*
