...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  RDIR=rpi-linux-$RVER
  if test ! -d $RDIR ; then
    git clone -b rpi-7.1.y https://github.com/raspberrypi/linux/ $RDIR
    test -d $RDIR || exit 1
  else
    pushd $RDIR
    git checkout rpi-7.1.y
    popd
  fi
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER 8d4e6356173a7b2e4a6a8ee1669060c33528fdb9
  cd ..
  rm -fr $RDIR
fi
...
sed -i -e '1 s/7.1.10-/7.1.11-/' $CDIR/debian/changelog
...
if test "$RPIPATCHES" = 1 ; then
  ...
  rm -f debian/abi/7.1.0-*/arm*
fi
rm -fr debian/abi/7.1.0-*
