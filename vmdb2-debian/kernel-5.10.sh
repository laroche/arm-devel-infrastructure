...
sed -i -e '1 s/5.10.217/5.10.218/' linux-5/debian/changelog
...
sed -i -e 's,^bugfix/all/tipc-fix-UAF-in-error-path.patch,,g' linux-5/debian/patches/series
sed -i -e 's,^bugfix/all/tipc-fix-a-possible-memleak-in-tipc_buf_append.patch,,g' linux-5/debian/patches/series
#sed -i -e 's,0128-sched-vtime-Consolidate-IRQ-time-accounting.patch,,g' linux-5/debian/patches-rt/series
...
if test "$RPIPATCHES" = 1 ; then
  ...
  pushd debian/patches
    ...
    #rm -f bugfix/rpi/0236-debugobjects-Make-RT-aware.patch
