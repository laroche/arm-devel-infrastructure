...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  if test ! -d rpi-linux-5 ; then
    git clone -b rpi-5.9.y https://github.com/raspberrypi/linux/ rpi-linux-5
  else
    pushd rpi-linux-5
    git checkout rpi-5.9.y
    popd
  fi
  cd rpi-linux-5 || exit 1
  git format-patch -o ../rpi-patches-$RVER 991a0920e1acc1244b81c4308407e6ac0c8e8bd9
  cd ..
  #rm -fr rpi-linux-5
fi
...
#sed -i -e '1 s/5.9.14-1/5.9.15-1/' linux-5/debian/changelog
