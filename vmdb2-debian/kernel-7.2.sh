...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  RDIR=rpi-linux-$RVER
  if test ! -d $RDIR ; then
    git clone -b rpi-7.2.y https://github.com/raspberrypi/linux/ $RDIR
    test -d $RDIR || exit 1
  else
    pushd $RDIR
    git checkout rpi-7.2.y
    popd
  fi
  cd $RDIR || exit 1
...
sed -i -e '1 s/7.2.4-/7.2.5-/' $CDIR/debian/changelog
...
if test "$RPIPATCHES" = 1 ; then
  ...
  rm -f debian/abi/7.2.0-*/arm*
fi
rm -fr debian/abi/7.2.0-*
