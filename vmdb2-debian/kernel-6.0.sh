...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  RDIR=rpi-linux-$RVER
  if test ! -d $RDIR ; then
    git clone -b rpi-6.0.y https://github.com/raspberrypi/linux/ $RDIR
    test -d $RDIR || exit 1
  else
    pushd $RDIR
    git checkout rpi-6.0.y
    popd
  fi
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER 31e4bdd2c25b50bca6d96995abb01a54ba5bd00e
  cd ..
  rm -fr $RDIR
fi
...
sed -i -e '1 s/6.0.11-/6.0.12-/' $CDIR/debian/changelog
...
if test "$RPIPATCHES" = 1 ; then
  ...
  rm -f debian/abi/6.0.0-*/arm*
fi
rm -fr debian/abi/6.0.0-*
