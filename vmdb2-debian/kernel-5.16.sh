...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  RDIR=rpi-linux-$RVER
  if test ! -d $RDIR ; then
    git clone -b rpi-5.16.y https://github.com/raspberrypi/linux/ $RDIR
    test -d $RDIR || exit 1
  else
    pushd $RDIR
  ...
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER 73be23491df081be0311e65c7744c9f1ec6ad412
  cd ..
  rm -fr $RDIR
fi
...
sed -i -e '1 s/5.16.19/5.16.20/' $CDIR/debian/changelog
