...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  RDIR=rpi-linux-$RVER
  if test ! -d $RDIR ; then
    git clone -b rpi-6.3.y https://github.com/raspberrypi/linux/ $RDIR
    test -d $RDIR || exit 1
  else
    pushd $RDIR
    git checkout rpi-6.3.y
    popd
  fi
  cd $RDIR || exit 1
  git format-patch -o ../rpi-patches-$RVER 429cff33b400edd76fc4d5e470742812a44fbc91
  cd ..
  rm -fr $RDIR
fi
...
sed -i -e '1 s/6.3.11-/6.3.13-/' $CDIR/debian/changelog
...
if test "$RPIPATCHES" = 1 ; then
  ...
  rm -f debian/abi/6.3.0-*/arm*
fi
rm -fr debian/abi/6.3.0-*
