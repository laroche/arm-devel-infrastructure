...
if test "$RPIPATCHES" = 1 -a ! -d rpi-patches-$RVER ; then
  ...
  RDIR=rpi-linux-$RVER
  if test ! -d $RDIR ; then
    git clone -b rpi-7.0.y https://github.com/raspberrypi/linux/ $RDIR
    test -d $RDIR || exit 1
  else
    pushd $RDIR
    git checkout rpi-7.0.y
    popd
  fi
  cd $RDIR || exit 1
...
if ! test -d $CDIR ; then
  ...
  git clone --single-branch --depth 1 -b debian/7.0/forky https://salsa.debian.org/kernel-team/linux.git $CDIR
  #git clone --single-branch --depth 1 -b 7.0-stable-updates https://salsa.debian.org/carnil/linux.git $CDIR
fi
#sed -i -e '/install-rtla)/d' $CDIR/debian/rules.real
# Change Debian source to new version:
sed -i -e '1 s/7.0.13-/7.0.14-/' $CDIR/debian/changelog
...
if test "$RPIPATCHES" = 1 ; then
  ...
  rm -f debian/abi/7.0.0-*/arm*
fi
rm -fr debian/abi/7.0.0-*
