...
if false && test "$HOSTTYPE" = "x86_64" && ! test -d /lib/modules/${KABI}-amd64 ; then
  ...
  wget -q https://github.com/laroche/arm-devel-infrastructure/releases/download/v20251129/$KERNEL
  tar xzf $KERNEL
...
if test "X$DEMOSETUP" = X1 ; then
  ...
  if test -d /lib/modules/6.1.0-48-amd64 ; then
    dpkg -P linux-image-6.1.0-48-amd64
  fi
  # Set new hostname:
