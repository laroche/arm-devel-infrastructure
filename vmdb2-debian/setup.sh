...
# incus launch images:ubuntu/25.10/cloud u2510 --config boot.autostart=true
# incus launch images:ubuntu/25.10/cloud u2510-vm --vm -p vm --config boot.autostart=true
#
...
if false && test "$HOSTTYPE" = "x86_64" && ! test -d /lib/modules/${KABI}-amd64 ; then
  ...
  wget -q https://github.com/laroche/arm-devel-infrastructure/releases/download/v20260322/$KERNEL
  tar xzf $KERNEL
...
if test "X$DEMOSETUP" = X1 ; then
  ...
  if test -d /lib/modules/6.1.0-42-amd64 ; then
    dpkg -P linux-image-6.1.0-42-amd64
  fi
  if test -d /lib/modules/6.12.73+deb13-amd64 ; then
    dpkg -P linux-image-6.12.73+deb13-amd64
  fi
  # Set new hostname:
